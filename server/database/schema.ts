...
export const photos = sqliteTable(
  ...
  (table) => [
    index('photos_date_taken_id_idx').on(table.dateTaken, table.id),
    index('photos_location_idx').on(table.latitude, table.longitude),
  ],
)
...
export const pipelineQueue = sqliteTable(
  ...
  (table) => [
    index('pipeline_queue_status_created_at_idx').on(
      table.status,
      table.createdAt,
    ),
    index('pipeline_queue_status_priority_created_at_idx').on(
      table.status,
      table.priority,
      table.createdAt,
    ),
    index('pipeline_queue_type_created_at_idx').on(
      sql`json_extract(${table.payload}, '$.type')`,
      table.createdAt,
    ),
  ],
)
...
export const photoReactions = sqliteTable(
  ...
  (table) => [
    index('photo_reactions_photo_type_idx').on(
      table.photoId,
      table.reactionType,
    ),
    ...
    index('photo_reactions_fingerprint_created_at_idx').on(
      table.fingerprint,
      table.createdAt,
    ),
  ],
...
export const albumPhotos = sqliteTable('album_photos', {
  ...
  albumPosition: index('album_photos_album_id_position_idx').on(
    table.albumId,
    table.position,
  ),
  photoId: index('album_photos_photo_id_idx').on(table.photoId),
}))
